...
const ShoppingModal = () => {
    ...
    return (
        <Modal open={showModal} onClose={onModalCloseHandler} sx={{
            display: 'flex',
            alignItems: 'center',
            ...
            <Sheet
                variant="outlined"
                sx={{borderRadius: 'md', p: 3, minWidth: '70vw', minHeight: '60vh'}}
            >
                <ModalDialog sx={{overflow: 'scroll', width: '100%', height: '100%', alignItems: 'center'}}
                             variant={'plain'}>
                    <ModalClose variant={'plain'} size={'lg'}/>
                    <ButtonBase onClick={() => {
                        openNewTab(mallProductUrl);
                    }}>
                        <Typography level="title-lg" variant="plain" color="neutral">상품명 : {productTitle}</Typography>
                    </ButtonBase>

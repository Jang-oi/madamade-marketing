...
const KeywordTemplate = () => {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        axiosAPI('/getkeyword', {mallProductUrl, productTitle}).then(res => {
    ...
    return (
        <div>
            <Table variant={"outlined"} sx={{width: '63vw', height: '45vh'}} borderAxis="both">
                <thead>
                <tr>
                    <th style={{textAlign: 'center'}} rowSpan={2}>연관 키워드</th>
                    <th style={{textAlign: 'center'}} colSpan={2}>월간 검색 수</th>
                    <th style={{textAlign: 'center'}} colSpan={2}>월평균 클릭 수</th>
                    <th style={{textAlign: 'center'}} colSpan={2}>월평균 클릭 률</th>
                    <th style={{textAlign: 'center'}} rowSpan={2}>경쟁 정도</th>
                    <th style={{textAlign: 'center'}} rowSpan={2}>총 상품수</th>
                    <th style={{textAlign: 'center'}} rowSpan={2}>월 평균노출 광고 수</th>
                    <th style={{textAlign: 'center'}} rowSpan={2}>모바일 웹 클릭 합계</th>
                    <th style={{textAlign: 'center'}} rowSpan={2}>검색 시 순위</th>
                </tr>
                <tr>
                ...
                <tbody>
                {keywordData && keywordData.map((data: any, index: any) => {
                    return (
                        <tr key={index}>
                            <td style={{cursor: 'pointer'}}
                                onClick={() => onKeywordClick(data.relKeyword)}>{data.relKeyword}</td>
                            ...
                            <td style={{textAlign: 'center'}}>{data.keywordRate ? setLocaleString(data.keywordRate) : '순위에 없음'}</td>
                        </tr>
                    );
                })}
                </tbody>
